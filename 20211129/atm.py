"""
Program: 20211129_atm
Author: Ayodele Olufemi
Date: 11/29/2021
Purpose: This modeule defines the ATM class, which provides a window for bank customers to perform deposits, withdrawals 
and check balances.
"""
...
class ATM(EasyFrame):
    ...
    def __init__(self, bank):
        ...
        self.statusField = self.addTextField(
            row=3, column=1, text="Welcome to the Bank!", state="readonly")
        self.balanceButton = self.addButton(
            row=0, column=2, text="Balance", command=self.getBalance, state="disabled")
        self.depositButton = self.addButton(
            row=1, column=2, text="Deposit", command=self.deposit, state="disabled")
        self.withdrawButton = self.addButton(
            row=2, column=2, text="Withdraw", command=self.withdraw, state="disabled")
        self.loginButton = self.addButton(
            row=3, column=2, text="Login", command=self.login)

    def login(self):
        """Attempts to login the customer. If successful, enables the buttons, included logout."""
    ...
    def getBalance(self):
        """Displays the current balance in teh status field."""
        text = "Balance = $" + str(self.account.getBalance())
        self.statusField.setText(text)

    def deposit(self):
        """Attempts a deposit. If not successful, displays error message in statusField; 
    ...
    def withdraw(self):
        """Attempts a withdrawal. If not successful, displays error message in statusField;
        otherwise, announces success."""
        amount = self.amountField.getNumber()
...
def main(fileName=None):
    """Creates the bank with the optional file name, wraps the window around it, 
    and opens the window. Saves the bank when the window closes. """
    if not fileName:
        bank = createBank(5)
    else:
        bank = Bank(fileName)
