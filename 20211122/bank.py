...
Date: 11/22/2021
Purpose: Create a class to hold savings account classes to create a class
"""
...
class Bank:
    ...
    An optional file name is also associated with the bank, to allow transfer
    of accounts to and from permanent file storage."""

    """The state of the bank is a dictionary of accounts and a file name. If 
    the file name is None, a file name for the bank has not yet been established."""
    ...
    def remove(self, name, pin):
        """Removes the account from the bank and returns it, or None if the account
        does not exist."""
        key = self.makeKey(name, pin)
        return self.accounts.pop(key, None)

    def get(self, name, pin):
        """Returns the account from the bank, or returns None if the account does
        not exist."""
        key = self.makeKey(name, pin)
    ...
    def save(self, fileName=None):
        """Saves pickled accounts to a file. The parameter allows the user to change 
        the file names."""
        if fileName != None:
            self.fileName = fileName
...
def createBank(numAccounts=1):
    ...
    names = ("Brandon", "Molly", "Elena", "Mark",
             "Tricia", "Ken", "Jill", "Jack")
    bank = Bank()
...
def main(number=10, fileName=None):
    """Creates and prints a bank, either from the optional file name argument
    or from the optional number."""
    testAccount()
    # if fileName:
    #     bank = Bank(fileName)
    # else:
